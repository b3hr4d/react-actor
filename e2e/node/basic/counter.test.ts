...
describe("counter", () => {
  ...
  it("should submit distinct requests with nonce by default", async () => {
    ...
    const values = await Promise.all(
      new Array(4).fill(undefined).map(() => counter.inc_read())
    )
...
describe("retrytimes", () => {
  it("should retry after a failure", async () => {
    ...
    const fetchMock = jest.fn(function (...args) {
      ...
      return fetch.apply(
        null,
        args as [input: string | Request, init?: RequestInit | undefined]
      )
    })

    const counter = await createActor({
